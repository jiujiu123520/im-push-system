...
# ---------------- 2. 修复权限并拉取最新代码 ----------------
echo "========== 1. 拉取最新代码 =========="
safe_chown "$OWNER_USER" "$PROJECT_DIR"
...
if grep -q "^GITHUB_TOKEN=" "$ENV_FILE" 2>/dev/null; then
    ...
    echo "如需修改,请手动编辑: $(has_sudo && [ "$(id -u)" -ne 0 ] && echo "sudo ")nano $ENV_FILE"
else
    echo "请准备以下信息:"
    ...
    run_sudo tee -a "$ENV_FILE" > /dev/null <<EOF
...
# 设置 .env 权限
run_sudo chown "$WEB_USER" "$ENV_FILE"
run_sudo chmod 600 "$ENV_FILE"
echo ""

# ---------------- 5. 设置 update_build_status.php 权限 ----------------
echo "========== 4. 设置 update_build_status.php 权限 =========="
UPDATE_STATUS_SCRIPT="$PROJECT_DIR/backend/bin/update_build_status.php"
if [ -f "$UPDATE_STATUS_SCRIPT" ]; then
    run_sudo chmod +x "$UPDATE_STATUS_SCRIPT"
    run_sudo chown "$WEB_USER" "$UPDATE_STATUS_SCRIPT"
...
# ---------------- 6. 确保输出和日志目录存在 ----------------
echo "========== 5. 确保构建目录存在 =========="
run_sudo mkdir -p "$PROJECT_DIR/build/output"
run_sudo mkdir -p "$PROJECT_DIR/build/logs"
safe_chown "$WEB_USER" "$PROJECT_DIR/build/output"
safe_chown "$WEB_USER" "$PROJECT_DIR/build/logs"
...
echo "     - SERVER_SSH_USER: ${OWNER_USER%%:*}"
echo "     - SERVER_SSH_KEY: SSH 私钥(完整内容)"
echo ""
echo "  2. 获取 keystore base64(在服务器执行):"
echo "     base64 -w 0 $PROJECT_DIR/build/keystore/release.keystore"
echo ""
